...
def watch_task(
    ...
    _verify_task_access(supabase, user_id=user_id, task_id=task_id)
    # Upsert-by-conflict: re-watching is a no-op.
    supabase.table("task_watchers").upsert(
        {"task_id": task_id, "user_id": user_id},
        on_conflict="task_id,user_id",
    ).execute()
    rows = (
        supabase.table("task_watchers")
        .select("*")
        .eq("task_id", task_id)
        .eq("user_id", user_id)
        .execute()
    ...
    row = rows[0]
    profile = _lookup_users(supabase, user_ids=[user_id]).get(user_id, {})
...
def unwatch_task(supabase: Client, *, user_id: str, task_id: str) -> None:
    _verify_task_access(supabase, user_id=user_id, task_id=task_id)
    supabase.table("task_watchers").delete().eq("task_id", task_id).eq(
        "user_id", user_id
    ).execute()
...
def list_task_watchers(
    ...
    profiles = _lookup_users(supabase, user_ids=user_ids)
    return [
        WatcherResponse(
            **r,
