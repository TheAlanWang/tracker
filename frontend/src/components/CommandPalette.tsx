  ...
  return (
    ...
    >
      ...
      <div className="fixed inset-0 z-50 flex items-start justify-center pt-[20vh] bg-black/30">
        <div className="w-full max-w-xl rounded-lg bg-white shadow-xl border border-slate-200 overflow-hidden">
          <Command.Input
            value={query}
...
function CommandItem({
  ...
  return (
    <Command.Item
      value={label}
      onSelect={onSelect}
      className="flex items-center justify-between rounded px-3 py-2 text-sm cursor-pointer text-slate-900 data-[selected=true]:bg-slate-100 aria-selected:bg-slate-100"
    >
      <span className="truncate">{label}</span>
      {sublabel && (
        <span className="ml-3 shrink-0 text-xs text-slate-400">{sublabel}</span>
      )}
    </Command.Item>
  );
}
